// 基于小根堆的定时器
// 在github.com\Terry-Mao\goim\libs\time基础上
// 增加超时函数参数，去掉log4go
package timer
...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			log.Printf("timer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
		}
	}
	if Debug {
...
func (t *Timer) expire() {
	...
	for {
		...
		t.lock.Unlock()
		if fn == nil {
			log.Printf("[WARN]expire timer no fn")
		} else {
			if Debug {
	...
	t.signal.Reset(d)
	if Debug {
		log.Printf("timer: expier reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	t.lock.Unlock()
	return
}
