...
func (t *Timer) add(td *TimerData) {
	...
	if td.index == 0 {
		...
		t.signal.Reset(d)
		if Debug {
			log.Println("timer: add reset delay %d ms", int64(d)/int64(time.Millisecond))
		}
	}
	if Debug {
		log.Println("timer: push item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}
...
func (t *Timer) del(td *TimerData) {
	...
	if i < 0 || i > last || t.timers[i] != td {
		...
		if Debug {
			log.Println("timer del i: %d, last: %d, %p", i, last, td)
		}
		return
	}
	if i != last {
	...
	if Debug {
		log.Println("timer: remove item key: %s, expire: %s, index: %d", td.Key, td.ExpireString(), td.index)
	}
	return
}
...
func (t *Timer) expire() {
	...
	for {
		...
		if fn == nil {
			...
			if Debug {
				log.Println("timer key: %s, expire: %s, index: %d expired, call fn", td.Key, td.ExpireString(), td.index)
			}
			fn()
		}
		t.lock.Lock()
	...
	t.signal.Reset(d)
	if Debug {
		log.Println("timer: expier reset delay %d ms", int64(d)/int64(time.Millisecond))
	}
	t.lock.Unlock()
	return
}
